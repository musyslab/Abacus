...
def serialize_submission(submission, *, include_grades: bool, teacher_view: bool = False):
    ...
    return {
        ...
        "points": submission.Points if include_grades else None,
        "feedback": submission.Feedback if include_grades else None,
        "adminGraderId": submission.AdminGraderId if include_grades else None,
        "hasSubmission": True,
...
def serialize_team_project_row(team, submission, *, include_grades: bool):
    ...
    if submission:
        ...
        points = submission.Points if include_grades else None
        feedback = submission.Feedback if include_grades else None
        admin_grader_id = submission.AdminGraderId if include_grades else None
...
def serialize_submission_history_item(submission):
    submitting_student = StudentUsers.query.get(int(getattr(submission, "StudentId", 0) or 0))
    team_id = int(getattr(submitting_student, "TeamId", 0) or 0) if submitting_student else None
...
# Returns all saved submissions for the same team + project, newest first.
# -----------------------------
...
def get_submission_history(submission_id):
    ...
    return jsonify({
        ...
        "projectId": project_id,
        "projectName": getattr(project, "Name", None) if project else None,
        "history": [serialize_submission_history_item(row) for row in history_rows],
    }), 200
