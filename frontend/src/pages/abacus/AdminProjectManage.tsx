...
export default function AdminProjectManage({
        ...
        await Promise.all(
            javaNames.map(async (name) => {
                try {
                    ...
                    const res = await fetch(url, authConfig())
                    if (!res.ok) return
    ...
    async function loadServerSolutionFiles() {
        try {
            const res = await axios.get(`${API}/projects/list_solution_files?id=${project_id}`, authConfig())
    ...
    async function openLocalPreview(file: File) {
        if (!SUPPORTED_RE.test(file.name)) {
            window.alert('Preview supports .py .java ')
            return
        }
    ...
    async function fetchServerFileList(): Promise<string[]> {
        const res = await fetch(`${API}/projects/list_source_files?project_id=${project_id}`, authConfig())
        if (!res.ok) return []
    ...
    async function handleJsonSubmit() {
        try {
            ...
            await axios.post(`${API}/projects/json_add_testcases`, formData, authConfig())
    ...
    function handleSolutionFilesChange(event: React.FormEvent) {
        ...
        if (arr.some(f => !SOLUTION_ALLOWED_RE.test(f.name))) {
            window.alert('Solution files must be Python (.py), Java (.java)')
            target.value = ''
            setSolutionFiles([])
            setSolutionFileNames([])
            setProjectLanguage(edit ? (serverProjectLanguageSnapshot || ProjectLanguage) : '')
    ...
    async function downloadAssignmentDescription() {
        try {
            const url = `${API}/projects/getAssignmentDescription?project_id=${project_id}`
    ...
    return (
        <>
                        ...
                        <div className="pane-testcases">
                            <div className="testcase-management-group">
                                ...
                                <div className="export-json-segment">
                                    ...
                                    >
                                        <FaDownload aria-hidden="true" /> Export test cases to JSON
                                    </button>
                                </div>
                                ...
                                <div className="json-button-group">
                                    ...
                                    >
                                        {submittingJson ? (
                                            ...
                                            <>
                                                <FaUpload aria-hidden="true" /> Submit JSON file
                                            </>
                                        )}
                                    </button>
                                </div>
            ...
            {modalOpen && (
                <>
                    <div
                        className="modal-overlay--testcase"
                        onClick={() => {
                            setModalOpen(false)
                            setModalDraft(null)
                        }}
                        ...
                        <div className="modal-content">
                            <form>
                                ...
                                <div className="form-field modal-textarea">
                                    <label>Input</label>
                                    <textarea
                                        className="modal-textarea textarea--input"
                                        rows={1}
