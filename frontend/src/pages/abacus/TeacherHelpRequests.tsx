...
const TeacherHelpRequests: React.FC = () => {
    const [helpRequests, setHelpRequests] = useState<HelpRequestItem[]>([])
    const [historyPage, setHistoryPage] = useState(1)

    const [selectedProblemId, setSelectedProblemId] = useState<string>('general')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!reason || !description.trim()) {
            setError('Please select a reason and describe your issue.')
            return
        }
    ...
    const canSubmitRequest = !!reason && !!description.trim()
...
export default TeacherHelpRequests